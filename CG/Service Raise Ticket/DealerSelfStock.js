...
function DealerSelfAndStock() {
    ...
    // useEffect(() => {
    //     fetch(`${process.env.REACT_APP_API_URL}Defect/GetDefectByProductLine?ProductLineCode=0`)
    //         .then((res) => res.json())
    //         .then((result) => {
    //             console.log(result);
    ...
    const handleShowCustomerTic = (cell) => {
        ...
        const getProdDetailUrl = `${process.env.REACT_APP_API_URL}ServiceTicket/GetServiceTicketByIdUnauthorize?ServiceTickeId=${cell?.row?.original?.msgCode}`;
    ...
    useEffect(() => {
        const cityUrl = `${process.env.REACT_APP_API_URL}Common/GetAllCommonUnauthorize?mode=4&Id=${DealerDetails?.stateId}&Code=0`;
        fetch(cityUrl)
            .then((res) => res.json())
            .then((result) => {
                console.log(result);
       ...
       //  let flatar=flatob
         ...
         if (e.target.files[0]) {
             ...
             reader.onload = async (e) => {
               ...
               //  })
               let dataList =[];
               let InvalidSerialNoLocal =[];
               let exstingTicketsLocal =[];
               let repeatedProductsLocal = [];
               let invalidDatesLocal = [];
               let isIvalidRec=false;
               for(let i=0;i<json.length;i++){
                let prodData = json[i];
                let getProdDetailUrl = `${process.env.REACT_APP_API_URL}ProdCustInfo/GetServiceTicketProdSerialNo?ProductSerialNo=${prodData["Product Sr"]}&${(prodData["Product Code/ Model"] && prodData["Product Code/ Model"] != 0) ? `ProductCode=${prodData["Product Code/ Model"]}` : ""}`;
                ...
                if(moment((prodData["InVoice date"].trim()?.split(" ")[0])).format("YYYY-MM-DD")  == "Invalid date"){
                    let arr= invalidDatesLocal.filter(obj => obj == prodData["Product Sr"]);
                            if(arr.length == 0){
                                invalidDatesLocal.push(prodData["Product Sr"]);
                            }
                    isIvalidRec=true;
                }                               
                else if(findRepeatedProducts.length > 0){
                    
                   let arr= repeatedProductsLocal.filter(obj => obj == prodData["Product Sr"]);
                            if(arr.length == 0){
                                repeatedProductsLocal.push(findRepeatedProducts[0].serialNo);
                            }
                    isIvalidRec=true;
                }
                    ...
                    let arr= repeatedProductsLocal.filter(obj => obj == prodData["Product Sr"]);
                            if(arr.length == 0){
                                repeatedProductsLocal.push(repeatedProductsinExcel[0].serialNo);
                            }
                    isIvalidRec=true;
                }
                else{
                    try {
                        ...
                        if (result?.msgCode === "SAP404") {
                            ...
                            // });
                            let arr= InvalidSerialNoLocal.filter(obj => obj == prodData["Product Sr"]);
                            if(arr.length == 0){
                                InvalidSerialNoLocal.push(prodData["Product Sr"]);
                            }
                            ...
                            // });
                            exstingTicketsLocal.filter(obj => obj == prodData["Product Sr"]);
                            if(exstingTicketsLocal.length == 0){
                                exstingTicketsLocal.push(prodData["Product Sr"]);
                            }
                            setloading(false);
                            isIvalidRec=true;
            ...
            //    let newAddedData = [...addedData, ...dataList];
               if(isIvalidRec){
                let msgToDisplay;
                if(repeatedProductsLocal.length > 0){
                    setRepeatedProducts(repeatedProductsLocal);
                    // msgToDisplay = "Serial Number/s: "+ repeatedProducts.join(", ")+" is already added in the system.";
                }
                else{
                    setRepeatedProducts([]);
                }
                if(InvalidSerialNoLocal.length != 0){
                    setInvalidSerialNo(InvalidSerialNoLocal);
                ...
                if(exstingTicketsLocal.length != 0){
                    setexstingTickets(exstingTicketsLocal);
                    // msgToDisplay = msgToDisplay+ "\n Service ticket is already raised for Serial number/s: "+ exstingTickets.join(", ");
                }
                else{
                    setexstingTickets([]);
                }
                if(invalidDatesLocal.length !=0){
                    setInvalidDates(invalidDatesLocal);
                }
                else{
                    setInvalidDates([]);
                }
               ...
               setfileContent(dataList)
               newob.ObjLeads.push(dataList)
    ...
    async function getAllASCpinCodewise(data, p_pinCode){
        try{
            let updatedData;
            let response = await fetch(`${process.env.REACT_APP_API_URL}CityPincodeMapping/GetAllPinCodeUserUnauthorizeGetAsc?Pincode=${p_pinCode}`);
    ...
    const handleAddButtonClick = async () => {
        ...
        !addInstallDetails.nameOfSpoc || !addInstallDetails.spocMobileNo  || !addInstallDetails.address1  || !addInstallDetails.prodInstallationPinId  || !addInstallDetails.prodInstallationStateId || !addInstallDetails.prodInstallationCityId ) {
            ...
            setloading(true)
            const getProdDetailUrl = `${process.env.REACT_APP_API_URL}ProdCustInfo/GetServiceTicketProdSerialNo?ProductSerialNo=${formData.serialNo}&${(formData.productCode && formData.productCode != 0) ? `ProductCode=${formData.productCode}` : ""}&DivisionCode=${selectedOption}`;
            ...
            try {
                ...
                if (result?.msgCode === "SAP404") {
                    ...
                    let Data = {
                        ...
                        distance: "",
                        nearestPinCode:"",
                        manufacturingDate:result?.prodSerialNoWarrantyList[0]?.manufacturingDate,
                        dateOfDispatch:result?.prodSerialNoWarrantyList[0]?.dateOfDispatch
                    };
                    let updatedData = Data;
                    if(Data.msg !="Service Ticket Available"){
                        updatedData = await getAllASCpinCodewise(Data, Data.prodInstallationPinId);
    ...
    const setMinimumInvoiceDate = (isSrNo, isProductCode, isProductType, value) => {
        ...
        if (formData.serialNo.length != "" && formData.productCode.length != "" && productType != "") {
            ...
            const getProdDetailUrl = `${process.env.REACT_APP_API_URL}ProdCustInfo/GetProdSerialNo?ProductSerialNo=${formData.serialNo}&${(formData.productCode && formData.productCode != 0) ? `ProductCode=${formData.productCode}` : ""}&DivisionCode=${productType}`;
            fetch(getProdDetailUrl)
                .then((res) => res.json())
                .then((result) => {
                    console.log(result);
                    ...
                    if (result.status == 404) {
                        ...
                        fetch(`${process.env.REACT_APP_API_URL}Issue/GetAllIssueUnauthorize?ProductLineCode=${result?.prodSerialNoWarrantyList[0].productLineCode}`)
                            .then((res) => res.json())
                            .then((result) => {
                                console.log(result);
                                setdefectsByPL(result)
    ...
    const handleSubmit = () => {
        ...
        let dealerRaiseTicketIP = {
            ...
            "objProdCustDeal": {
                ...
                "isActive": true,
                "redirectingFrom": localStorage.getItem("RedirectedFrom"),
        ...
        const getProdDetailUrl = `${process.env.REACT_APP_API_URL}ProdCustInfo/UpsertProductCustomerDealer`;
        ...
        fetch(getProdDetailUrl, {
            ...
            .then((result) => {
                if (result?.Key == 500) {
                    ...
                    fetch(process.env.REACT_APP_API_URL + "ServiceTicket/GetServiceTicketByIdUnauthorize?ServiceTickeId=" + result)
                        .then((res) => res.json())
                        .then((result) => {
                            if (result?.Key == 500) {
                                Swal.fire({
                        ...
                        <div style={{
                            ...
                            {/* <div>
                                ...
                                <>
                                    ...
                                    <h4>Welcome{RedirectionRouteSelfStock === "EndCustomer" ? dealername : "retailerName"}</h4>
                            ...
                            <Tab eventKey={'0'}>
                                <Row className="justify-content-start m-0  " >
                                    ...
                                    <Row className="mt-2 text-start">
                                        ...
                                        :
                                            <>
                                                ...
                                                <Form.Group >
                                                    <Form.Label>
                                                    Partner Name <span className="req-t">*</span>
                                                    </Form.Label>
                                                    <Form.Control
                                                        type="text"
                                                        readOnly
                                                ...
                                                <Col md={2}>
                                                    <Form.Group >
                                                        <Form.Label>
                                                        Partner Mobile No <span className="req-t">*</span>
                                                        </Form.Label>
                                                        <Form.Control
                                                        readOnly
                                                            type="text"
                                        ...
                                        <Col className="" md={2}>
                                            <Form.Group >
                                                ...
                                                <Form.Control
                                                    ...
                                                    onChange={(e) => {
                                                        setaddInstallDetails({ ...addInstallDetails, prodInstallationPinId: e.target.value })
                                    ...
                                    <Col  md={9} className="mt-2">
                                            {/* Upload ASC Excel * Note: Please check Provided Excel format before uploading!  */}
                                            <div className='d-flex justify-content-between'>

                        
                        <div className='d-flex justify-content-between'>

                        <p className='pg-label m-0'>Upload product list Excel <span className='mx-3' style={{fontWeight:"400",fontSize:"14px"}}><u><span className='req-t'>*</span> Note: <a href={ProductListFormat} download="Bulk Import" target='_blank'>Download </a> excel template to add products in bulk! <span className='req-t'>*</span></u></span></p>
          ...
          {!xlFileName &&  (<Button
                  ...
                  {xlFileName &&  <Button variant="" className='up-Btn' onClick={(e)=>{
                    ...
                    if(xlFileName.endsWith(".xls") || xlFileName.endsWith(".xlsx")){
                        let newAddedData = [...addedData, ...fileContent];
                        setAddedData(newAddedData);
                        setxlFileName("")
                        xlInput.current.value="";
                    }
                    ...
                    enableTopToolbar={false}
                                ...
                                <Modal show={showRegistered} onHide={handleRegisteredClose} size="lg" centered >
                                    <Modal.Body>
                                        ...
                                        {repeatedProducts.length !=0 && <p >Serial Number's: {repeatedProducts.join(", ")} already added in the list.</p>}
                                        {InvalidSerialNo.length !=0 && <p > Serial no / product code not matching, for Serial number's: {InvalidSerialNo.join(", ")}</p>}
                                        {exstingTickets.length !=0 && <p> Service request's already available against given serial number's: {exstingTickets.join(", ")}</p>}
                                        {invalidDates.length !=0 && <p> Please check dates for Serial number's: {invalidDates.join(", ")}</p>}
                                    </Modal.Body>
...
export default DealerSelfAndStock